...
| `VLLM_OPENVINO_FAST_SAMPLER` | CPU only. Replace the Gumbel-max noise draw in vLLM V1's `compiled_random_sample` with a numpy PCG64 fill (~3.7× faster on decode shapes). Set to `0` to use stock `torch.exponential_()`. See [Sampling Parameters](#sampling-parameters-large-vocab-models) for the seed caveat. | `1` |
...
| `VLLM_OPENVINO_FAST_SAMPLER` | int | `1` (default), `0` | `1` uses the numpy PCG64 fast path for the sampler's Gumbel-max noise draw; `0` restores stock `torch.exponential_()` |
...
**Plugin numpy PCG64 fast-sampler (`VLLM_OPENVINO_FAST_SAMPLER`, default `1`)**: for non-greedy decoding, upstream `compiled_random_sample` (`vllm/v1/sample/ops/topk_topp_sampler.py`) draws its Gumbel-max noise with `torch.empty_like(probs).exponential_()`, a single-threaded scalar inverse-CDF fill. The plugin replaces that one call with `numpy.random.Generator.standard_exponential(dtype=np.float32)` (PCG64), which fills the same `(batch, vocab)` buffer ~3.7× faster (task-14 microbench); the rest of the Gumbel-max math (softmax → div → argmax) is unchanged. Measured on LFM2.5-350M-int8-ov at concurrency 8, `NUM_STREAMS=1`, `CPU_THREADS_NUM=24`, temp=0.7, `ignore_eos=true` so all 8 requests emit exactly 128 tokens (removing the RNG-dependent completion-length confound): 3-run mean aggregate **121.13 tok/s baseline vs 168.68 tok/s patched — 1.39×** (sample stddev 2.86 / 3.73), at the top of the task-14 Amdahl projection band (1.24–1.40×). On dense Qwen3-1.7B-int4-ov at the same settings (vocab 151,936, PagedAttention path), 3-run mean aggregate **29.50 tok/s baseline vs 36.00 tok/s patched — 1.22×** (sample stddev 1.67 / 1.56), matching that model's task-14 Amdahl prediction (~1.22×, a ~25% sampler share × 3.7× on that share). On hybrid Qwen3.5-0.8B-int4-ov (text-only, Hybrid-PA path, vocab 248,320) at the same settings, 3-run mean aggregate **35.38 tok/s baseline vs 56.15 tok/s patched — 1.59×** (sample stddev 0.36 / 3.29), the largest gain measured because the model's small per-step inference cost and large vocabulary give it the highest sampler share (~50%). Set `VLLM_OPENVINO_FAST_SAMPLER=0` to fall back to stock `torch.exponential_()`. Seed caveat: swapping the RNG changes the token stream a user-supplied `seed` produces relative to stock vLLM, but upstream V1 CPU decoding already produces different tokens on repeated seeded temp>0 requests (verified on both baseline and patched containers); the patch adds no new nondeterminism, and temp=0 output remains bit-identical. The patch never fires for a batch where every request carries its own seed, since upstream routes those through the per-request `torch.Generator` path this patch does not touch. The patch also tries `mkl_random` first (when the MKL shared libs are present, e.g. in a custom image with `mkl` + `mkl-random` + `intel-openmp` installed); MKL's `standard_exponential` is ~2× faster than numpy PCG64 on the same shapes, and the fallback to numpy PCG64 is automatic when MKL is absent.

**Vision-embedding cache**: for multimodal models (Qwen3.5-VL, Gemma-4), the plugin caches merged vision embeddings keyed by `mm_feature.identifier` (a stable blake3 content hash of the image). The cache is an LRU bounded at 8 entries (~32 MB max), lives in `OpenVINOCausalLM._vision_embed_cache`, and is transparent to the caller — no configuration needed. When the same image appears in multiple requests (concurrent duplicates, multi-turn with a different prefix, post-eviction re-prefill), the OpenVINO vision tower + merger are skipped entirely. Measured on Qwen3.5-0.8B-int4-ov (Hybrid-PA, 1024×1024 image, 1024 merged tokens): TTFT dropped from 11.82 s (cache miss) to 6.46 s (cache hit) — 1.83× faster. For small images (64 merged tokens) the gain is smaller (0.72 s → 0.60 s). The cache complements vLLM's KV prefix caching: when the image-containing prefix is already cached in KV blocks, vision is skipped regardless; the embedding cache catches the cases where the prefix differs but the image is the same.
