...
from vllm_openvino.utils import cpu_thread_limit, has_sliding_window
...
class OpenVINOCausalLM(nn.Module):
    ...
    def __init__(
        ...
        if ov_device_upper == "CPU":
            ...
            cpu_threads_num, thread_limit_reason = cpu_thread_limit()
            if cpu_threads_num:
                logger.info(
                    "[OV-LOADER] Capping OpenVINO inference threads to %d (%s). "
                    "Set VLLM_OPENVINO_CPU_THREADS_NUM explicitly to override.",
                    cpu_threads_num, thread_limit_reason)
        ...
        # Merged vision embeddings keyed by mm content hash. OpenVINO reuses
        # output buffers across infer calls, so values are detached copies.
        self._vision_embed_cache: OrderedDict[str, np.ndarray] = OrderedDict()
...
class HybridPAInputBuilder(OpenVINOInputBuilder):
    ...
    def __init__(self, model: "OpenVINOCausalLM") -> None:
        self.model = model
        # Pre-allocated 0-dim buffer for max_context_len.
        # Model (PA-transformed) expects scalar shape=[]; attn_metadata provides
