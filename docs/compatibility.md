...
25. **Hybrid-PA 세그폴트 회귀 수정 (2026-08-18)** — 항목 23의 "크래시 없이 동작, byte-exact 검증 완료"는 검증 이후 커밋 `96862df`(리팩토링)에서 재발한 회귀로 무효화되어 있었음. `HybridPAInputBuilder`가 `max_context_len`을 매 스텝 갱신하기 위해 `ov.Tensor(_buf)` (1-인자 생성자, `shared_memory=False`가 기본값이라 버퍼를 복사함)로 텐서를 만들고 `_buf[()] = ...`로 in-place 갱신을 시도했으나, 텐서가 원본 버퍼를 공유하지 않아 OpenVINO가 항상 `max_context_len=0`을 읽음 → CPU PagedAttention 커널(`executor_pa.cpp`)이 0-크기 attention weight 스크래치 버퍼를 할당하고 그 위에 쓰기를 시도해 `ov::InferRequest::infer()` 내부에서 네이티브 세그폴트 발생. Qwen3.5, LFM2.5 모두 **단일 요청**만으로도 재현됨(동시성과 무관). `ov.Tensor(_buf, shared_memory=True)`로 수정 — 단일/연속/4-동시 요청 모두 podman에서 재검증 완료. 별개로, `get_kv_cache_spec()`이 아직 미해석 상태인 `cache_config.cache_dtype`("dynamic")로 KV 캐시 dtype을 판단해 실제 컴파일된 모델의 dtype(`self.cache_dtype`, 예: `u8`)과 불일치하던 버그도 함께 수정 — `determine_available_memory()`와 동일한 해석 순서로 통일.
